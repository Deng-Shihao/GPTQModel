...
def load_pack_block_extension(*, verbose: bool = False) -> Optional[object]:
    ...
    if hasattr(torch.ops.awequant, "pack_block_cpu"):
    ...
    try:
        spec = importlib.util.find_spec("awequant_pack_block_cpu")
    ...
    if not source_path.exists():
        source_path = project_root / "awequant_ext" / "pack_block_cpu.cpp"
    if not source_path.exists():
    ...
    build_dir = os.getenv("AWEQUANT_EXT_BUILD")

    if not verbose:
        verbose = env_flag("AWEQUANT_EXT_VERBOSE", True)

    try:
        safe_load_cpp_ext(
            name="awequant_pack_block_cpu",
